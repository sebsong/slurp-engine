...
namespace platform {
    ...
    PLATFORM_GET_LOCAL_FILE_PATH(getLocalFilePath) {
        return std::filesystem::path(SDL_GetBasePath()).replace_filename(filename).string();
    }

#if DEBUG
    PLATFORM_DEBUG_TOGGLE_PAUSE(DEBUG_togglePause) {
        GlobalIsPaused = !GlobalIsPaused;
    }
#endif

    PLATFORM_VIBRATE_GAMEPAD(vibrateGamepad) {
        // TODO: impl
    }
    ...
    static void allocateMemoryArenas(memory::MemoryArena& outPermanentMemory, memory::MemoryArena& outTransientMemory) {
        ...
        types::byte* memory = allocateMemory(size);
    ...
    static PlatformDll loadPlatformLib() {
        PlatformDll platformLib = {};
        platformLib.getLocalFilePath = getLocalFilePath;
        platformLib.vibrateGamepad = vibrateGamepad;
        platformLib.shutdown = shutdown;
#if DEBUG
        platformLib.DEBUG_togglePause = DEBUG_togglePause;
#endif
        return platformLib;
    }
